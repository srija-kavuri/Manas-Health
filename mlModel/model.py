...
def predict_severity(user_inputs):
    try:
        df = pd.read_csv('manashealth/mlmodel/dataset of anxiety.csv')
        ...
        # Ensure that the column name 'Multiyply by 2' exists in new_instance
        if 'Multiyply by 2' not in new_instance.columns:
            raise ValueError("Column 'Multiyply by 2' is missing in user inputs.")

        predicted_severity = le.inverse_transform(model.predict(new_instance[['Multiyply by 2']]))[0]
        return predicted_severity

    except Exception as e:
        print("Error in predict_severity:", str(e))
